...
**Continue Session B as the architect** — judgment work. The architect proposes a
plan and approves it; `taskspec` validates and generates. The developer role
waits for checkpoint 06.
...
```bash
taskspec version
taskspec init        # creates .taskspec/config and tasks/ — skip if tasks/ exists
```
...
NÃO escreva nenhum Task-Spec. Escreva um único manifesto TaskPlan/v1 em
tasks/.plans/transform-5-8.yaml com approved: false, seguindo o schema de
~/GitHub/task-spec/docs/examples/task-plan.yaml.
...
Then generate. Try it once *before* flipping if you want the beat — `batch`
...
The architect must refuse to give it a definition. Record it as a hole rather
than a build instruction, and make the refusal machine-visible:
...
4. **`taskspec ready`** — the frontier, with the revenue unit absent. Then
   `tasks/_metrics.jsonl`, whose last line is the timestamped status change
   carrying the reason and the owner.
...
- Item 10 was requested and refused; it is `blocked`, `taskspec ready` excludes
  it, and `tasks/_metrics.jsonl` records the reason with Finance named.
...
1. **`status: blocked` in the manifest is silently ignored.** A unit declared
   ...
   `taskspec ready`. The only path to blocked is `taskspec transition` after
   generation, which is why move 5 runs it explicitly.
...
If `taskspec` misbehaves on the night, fall back to the manual path: ask the
architect to write the five specs and `tasks/_state.yaml` directly, exactly as
checkpoint 02 was written by hand. Announce the fallback as **prepared** and say
plainly that the tool is what the room is being given, not what the lesson
depends on.
